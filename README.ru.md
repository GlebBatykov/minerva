...
Dart фреймворк для создания серверов
...
Вы можете загрузить на сервер файл при путем посылки файла в `FormData`. Далее при обработке запроса вы можете воспользоваться методом asForm экземпляра класса `RequestBody`, для того чтобы получить `FormData` данные из входящего запроса. Далее вы можете по имени поля в `FormData`, получить экземпляр класса `FormDataFile`. Экземпляр класса `FormDataFile` содержит имя файла, а так же содержимое файла в виде байт.

Пример получения файла при помощи `FormData`, записи файла по произвольному пути при помощи библиотеки `dart:io`:
...
Отдать на скачивание файл вы можете либо самостоятельно настроив заголовки ответа, а так же передав содержимое файла в ответ, при помощи класса `Result`. Либо воспользоваться готовыми результатами, такими как: `FileResult`, `FilePathResult`.

Чтобы отправить файл при помощи `FileResult` вы должны передать в него экземпляр класса `File` из библиотеки `dart:io`.

Чтобы отправить файл при помощи `FilePathResult` вы должны указать путь к файлу. Путь к файлу может быть задан как абсолютно, так и относительно папки проекта.

Пример использования `FileResult` для отдачи на скачивания файла:
...
Minerva содержит 2 вида сборки проекта: `debug` и `release`.

Вы можете собрать проект в `release` сборке при помощи команды:

```dart
minerva build -m release
```

Проект будет собран в соответствии с настройками заданными в файле `appsetting.json` в корне проекта.

После сборки в папке вашего проекта, в папке `build` вы увидите папку `release`. Далее вы можете использовать эту папку и её содержимое для развертывания сервера.

Файл `details.json` в папке сборки проекта вы можете удалить, так как он полезен только при разработке проекта, при пересборке проекта.

## Docker контейнер

Когда вы создаете Minerva проект, в корне вашего проекта создается `Dockerfile`, для сборки `Docker image`.

Существует два шаблона `Docker` файлов в Minerva: `AOT` и `JIT`. Шаблоны для соответствующих типов компиляции, так как их развертывание в `Docker контейнере` отличается.

По умолчанию Minerva создает `Dockerfile` для `AOT` компиляции, однако вы можете указать необходимый тип компиляции `Dockerfile` при создании проекта.

Во время разработки вы можете добавлять произвольные файлы в сборку вашего проекта при помощи параметра `assets` в файле `appsetting.json`.

Если вы добавляли какие либо файлы в папку `asset` (которая создается в шаблонном `Minerva` проекте), или добавляли какие либо другие ассеты в файл `appsetting.json`, то перед созданием `Docker image` вы должны - перегененировать `Dockerfile`.

Перегенерировать `Dockerfile` вы можете при помощи одно команды - команды `docker` в `CLI` утилите `Minerva` фреймворка.

Эта команда перегенерирует `Dockerfile` и добавит в него строки необходимые для добавления всех ваших ассетов в конечный `Docker image`.

Команда по умолчанию генерирует `Dockerfile` для `AOT` компиляции проекта, но вы можете указать необходимый вам тип компиляции.

Команда генерации `Dockerfile` для `AOT` компиляции:

```dart
minerva docker -c AOT
```

Команда генерации `Dockerfile` для `JIT` компиляции:
...
- 🚧 Ускорить сериализацию и десериализацию JSON;
- 🚧 Убрать ошибки при работе с multipart/form-data и ускорить время парсинга multipart/form-data;
- 🚧 Покрыть тестами;
